...
namespace RigelEditor.Platforms
{
    ...
    internal class GUIGraphicsBindSharpDX : IDisposable, IGUIGraphicsBind
    {
        ...
        bool IGUIGraphicsBind.NeedRebuildCommandList { get => throw new NotImplementedException(); set => throw new NotImplementedException(); }
        ...
        public GUIGraphicsBindSharpDX(GraphicsContext graphics)
        {
            m_graphics = graphics;
        ...
        public void SyncDrawTarget(GUIDrawStage stage, GUIDrawTarget drawtarget)
        {
            ...
            if(stage is GUIDrawStageMain)
            {
                ...
                BufferIndices.CheckAndExtendsWithSize(Mathf.Max(rectCount, textCount) / 4 * 6);

                return;
            }


            if(stage is GUIDrawStageOverlay)
            {
                //rect
                {
                    var rectCount = drawtarget.bufferRect.Count;
